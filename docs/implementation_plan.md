...
#### 무한 로딩 버그 수정 (2026-02-28)
- **증상**: 다중 디바이스(PC, 모바일)에서 동시 접속 시 카카오/구글 로그인 성공 후 홈 화면에서 무한 대기(로딩 중...) 상태 지정.
- **원인**: `AuthProvider`에서 로그인 후 `isInitialized`가 완료되어 세션 객체(`user`)는 생성되었지만, `users` 테이블에서 `dbUser`를 가져오는 시점이 지연되어 `user`는 존재하는데 `dbUser`는 null인 상태(`isDbUserLoading` 상태)가 지속됨. 이로 인해 `useQuizpacks`와 `page.tsx`에서 무한 로딩에 빠짐.
- **해결 방안**:
  1. `AuthProvider.tsx` 내부 상태에 `isDbUserLoaded` 플래그 추가 (초기값 `false`).
  2. `fetchDbUser`의 API 호출이 끝난 시점(null 반환 포함)에 `isDbUserLoaded`를 `true`로 갱신하여 데이터베이스 사용자 조회가 완결되었음을 보장.
  3. `useQuizpacks.ts` 및 홈 화면(`page.tsx`) 로직에서 `dbUser`가 없더라도 조회가 1회 완결(`isDbUserLoaded === true`)되었다면, 추가 로딩을 중단하고 빈 목록 또는 온보딩 리다이렉트를 처리하게 함.

#### 진행 중인 작업: UI/UX 개선 (퀴즈팩 카드 스타일 개편)
디자인 시안을 바탕으로 `QuizpackCard`의 상태별 UI를 직관적이고 모던하게 수정합니다.

**1. 상태별 스타일 변경 상세**
| 상태 | 컨테이너 룩 | 상태 아이콘 (우측 상단) | 번호 색상 | 태그 스타일 | 레이아웃 / 하단 텍스트 |
|------|-----------|------------------------|----------|------------|-----------------------|
| **닫힘** (`closed`) | 배경 `#F3F4F6`<br>테두리 있음 | 자물쇠 (`Lock`, 회색) | 회색 (`text-gray-400`) | 배경 `#E5E7EB`<br>글씨 `#9CA3AF` | 하단: "열리지 않았어요" (회색 텍스트) |
| **열림** (`opened`) | 배경 `#FFFFFF`<br>연회색 테두리 (`border-gray-200`) | 재생 버튼 (`Play`, 주황색 선) | 주황색 (`text-[#FF8400]`) | 배경 `#2D2D2D`<br>글씨 `#FF8400` | 하단: "이제 풀 수 있어요" (주황색 텍스트) |
| **진행중** (`in_progress`) | 배경 `#FFFFFF`<br>주황색 테두리 (`border-[#FF8400]`) | 일시정지 (`Pause`, 주황색 선) | 주황색 (`text-[#FF8400]`) | 배경 `#2D2D2D`<br>글씨 `#FF8400` | 중간: “풀고 있는 중이에요” 우측 별점<br>하단: 진행상황 바 (주황색/회색) + 우측 % 텍스트 |
| **완료** (`completed`) | 배경 `#FFFFFF`<br>연회색 테두리 (`border-gray-200`) | 체크 원 (`CheckCircle`, 굵은 검정) | 검정색 (`text-[#2D2D2D]`) | 배경 `#2D2D2D`<br>글씨 `#D1D5DB` | 하단: "정답률 XX%" (주황색)<br>우측 하단: 주황색 별 아이콘 + 별점 수치 |

**2. 구현 방식 변경**
- 인라인 스타일(`style={{}}`) 최소화 및 Tailwind CSS 클래스 적극 활용
- 조건부 렌더링 최적화 (`cn()` 유틸리티 활용 고려 또는 템플릿 리터럴 가독성 개선)
- `lucide-react` 아이콘의 `strokeWidth` 조절을 통해 디자인 시안의 얇고 두꺼운 특성 표현
